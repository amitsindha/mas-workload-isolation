#!/usr/bin/env bash
# MAS Workload Isolation Orchestrator V4.3.2
# Validated pattern based on EAM1 MAS 9.1.22 POC.
# V4.3.2 corrections:
#   - FacilitiesApp convergence no longer depends on .status.podTemplates, which is
#     not populated by all Facilities operator versions. Validate CR spec, Ready,
#     generated workload affinity/rollout, and actual placement instead.
#   - FacilitiesWorkspace podTemplate keys are discovered from active controllers
#     owned by the FacilitiesWorkspace and filtered against IBM-supported keys.
#     This covers appserver, multiagents, reportqueueagent, wfagent and other active agents.
# V4.3.1 correction:
#   - Add missing node_allowed() helper used by AppConfig/Graphite validation.
#     No change to the proven MAS/ManageServerBundle isolation logic.
# V4.3 improvements:
#   1. Health gates use CURRENT workload health; historical FailedScheduling events are diagnostic only.
#   2. Running pod readiness detects partial readiness (for example 1/2), not only 0/N.
#   3. Existing V4.2 isolation, capacity, Facilities, SLS, MongoDB and DB2 behavior retained.
#
# V4.2 baseline improvements:
...
usage(){ cat <<'EOF'
Usage:
 ./mas-workload-isolation-V4.3.2.sh --components mas|sls|mongodb|db2|facilities|comma-separated combination --instance eam2 --core-namespace mas-eam2-core \
...
node_allowed(){ local node="$1" n; for n in "${NODES[@]}"; do [[ "$node" == "$n" ]] && return 0; done; return 1; }
...
facilities_controller_affinity_ok(){
  local kind="$1" name="$2"
  oc --request-timeout="$OC_REQUEST_TIMEOUT" get "$kind" "$name" -n "$FACILITIES_NS" -o json 2>/dev/null | jq -e \
...
facilities_controller_placement_ok(){
  local kind="$1" name="$2" selector bad
  selector="$(oc --request-timeout="$OC_REQUEST_TIMEOUT" get "$kind" "$name" -n "$FACILITIES_NS" \
    -o jsonpath='{range .spec.selector.matchLabels[*]}{@}{end}' 2>/dev/null || true)"
  # Controller selector serialization is not reliable across resource types; use owner/prefix
  # validation later as the authoritative placement gate.
  [[ -n "$name" ]]
}

facilities_cr_apply_no_status_wait(){
  local tag="$1" kind="$2" name="$3" ns="$4"; shift 4
  local keys=("$@") a=() k f="$RUN_DIR/patches/$tag.json"
  if ! $FORCE_REAPPLY && all_cr_keys_compliant "$kind" "$name" "$ns" "${keys[@]}"; then
    compliance_log "$tag" COMPLIANT SKIPPED
    info "$tag already compliant; skipping patch"
    return 0
  fi
  compliance_log "$tag" DRIFT/NEW APPLY
  backup "$kind" "$name" "$ns" "$tag-before.yaml"
  for k in "${keys[@]}"; do a+=("$(entry "$k")"); done
  mkpatch "$f" "${a[@]}"
  patch "$kind" "$name" "$ns" "$f"
}

facilities_workspace_active_keys(){
  local ws="$1"
  local supported='^(appserver|multiagents|dataconnectagent|extendedformulaagent|formularecalcagent|incomingmailagent|objectmigrationagent|objectpublishagent|maintenanceagent|reportqueueagent|reservesmtpagent|scheduleragent|wfagent|wffutureagent|wfnotificationagent)$'
  oc --request-timeout="$OC_REQUEST_TIMEOUT" get deployment,statefulset -n "$FACILITIES_NS" -o json 2>/dev/null | jq -r --arg ws "$ws" '
    .items[]
    | select(any(.metadata.ownerReferences[]?; .kind=="FacilitiesWorkspace" and .name==$ws))
    | .metadata.name
  ' | while read -r controller; do
        [[ "$controller" == "$ws-"* ]] || continue
        key="${controller#"$ws-"}"
        [[ "$key" =~ $supported ]] && printf '%s\n' "$key"
      done | sort -u
}
...
facilities_phase(){
  ...
  local fapp ws bad key kind controller valid
  local -a app_keys=(entitymgr-ws usersyncagent)
  local -a ws_keys=()
  local -a ws_patch_keys=()

  fapp="$(oc get facilitiesapp -n "$FACILITIES_NS" -l "mas.ibm.com/instanceId=$INSTANCE" -o jsonpath='{.items[0].metadata.name}' 2>/dev/null || true)"
  [[ -n "$fapp" ]] || die "FacilitiesApp not found in $FACILITIES_NS"

  # FacilitiesApp status.podTemplates is not populated by all operator versions.
  # Apply/verify the supported CR spec, then validate Ready + generated workloads.
  facilities_cr_apply_no_status_wait "facilitiesapp-$fapp" facilitiesapp "$fapp" "$FACILITIES_NS" "${app_keys[@]}"
  $DRY_RUN && return 0

  wait_for "FacilitiesApp/$fapp Ready" bash -c "test \"\$(oc get facilitiesapp '$fapp' -n '$FACILITIES_NS' -o json | jq -r '[.status.conditions[]?|select(.type==\"Ready\")][0].status // \"False\"')\" = True"

  for key in "${app_keys[@]}"; do
    controller="${INSTANCE}-${key}"
    wait_for "FacilitiesApp/$fapp $key affinity" facilities_controller_affinity_ok deployment "$controller"
    oc rollout status "deployment/$controller" -n "$FACILITIES_NS" --timeout="${TIMEOUT}s" \
      || die "FacilitiesApp workload rollout failed: deployment/$controller"
  done
  ...
  for ws in "${_fws[@]}"; do
    [[ -n "$ws" ]] || continue
    mapfile -t ws_keys < <(facilities_workspace_active_keys "$ws")
    ((${#ws_keys[@]} > 0)) || die "No active supported FacilitiesWorkspace controllers discovered for $ws"
    # datainit is a supported one-shot Job rather than a persistent controller. Keep it
    # configured so future data-initialization runs inherit the isolation policy.
    mapfile -t ws_patch_keys < <(printf '%s\n' datainit "${ws_keys[@]}" | sort -u)
    info "FacilitiesWorkspace/$ws active supported podTemplate keys: ${ws_keys[*]}"
    info "FacilitiesWorkspace/$ws configured podTemplate keys (includes future datainit): ${ws_patch_keys[*]}"

    facilities_cr_apply_no_status_wait "facilitiesworkspace-$ws" facilitiesworkspace "$ws" "$FACILITIES_NS" "${ws_patch_keys[@]}"
    ...
    # If PodTemplatesValid is published, False is blocking. Missing is acceptable because
    # some Facilities operator versions do not expose this condition consistently.
    valid="$(oc get facilitiesworkspace "$ws" -n "$FACILITIES_NS" -o json | jq -r '[.status.conditions[]?|select(.type=="PodTemplatesValid")][0].status // "Missing"')"
    [[ "$valid" != "False" ]] || die "FacilitiesWorkspace/$ws reports PodTemplatesValid=False"

    for key in "${ws_keys[@]}"; do
      controller="${ws}-${key}"
      if oc get statefulset "$controller" -n "$FACILITIES_NS" >/dev/null 2>&1; then
        kind=statefulset
        wait_for "FacilitiesWorkspace/$ws $key affinity" facilities_controller_affinity_ok "$kind" "$controller"
        oc rollout status "statefulset/$controller" -n "$FACILITIES_NS" --timeout="${TIMEOUT}s" \
          || die "FacilitiesWorkspace workload rollout failed: statefulset/$controller"
      elif oc get deployment "$controller" -n "$FACILITIES_NS" >/dev/null 2>&1; then
        kind=deployment
        wait_for "FacilitiesWorkspace/$ws $key affinity" facilities_controller_affinity_ok "$kind" "$controller"
        oc rollout status "deployment/$controller" -n "$FACILITIES_NS" --timeout="${TIMEOUT}s" \
          || die "FacilitiesWorkspace workload rollout failed: deployment/$controller"
      else
        die "Generated FacilitiesWorkspace controller not found for key '$key' (expected $controller)"
      fi
    done
  done
  ...
  while true; do
    ...
    for ns in "$CORE_NS" "$MANAGE_NS"; do
      ...
      while IFS='|' read -r pod phasev waiting ready node; do
        ...
        # V4.3: treat any partial readiness as unhealthy (0/1, 1/2, etc.).
        # Empty containerStatuses (0/0) is not independently treated as NotReady;
        # Pending/waiting state above remains authoritative during pod startup.
        ready_count="${ready%%/*}"
        total_count="${ready##*/}"
        if [[ "$ready_count" =~ ^[0-9]+$ && "$total_count" =~ ^[0-9]+$ ]] && \
           (( total_count > 0 && ready_count < total_count )); then
          issue="${issue:-NotReady}"
        fi
      ...
      # V4.3: FailedScheduling events are diagnostic evidence, not a health gate
      # by themselves. Kubernetes events outlive the scheduling condition and can
      # remain after a replacement/current pod is Running and Ready. Current Pending
      # pods are already blocking above, so stale events must not keep bad=1 forever.
      while IFS='|' read -r obj msg; do
        [[ "$obj" == "${INSTANCE}-"* ]] || continue
        current_phase="$(oc get pod "$obj" -n "$ns" -o jsonpath='{.status.phase}' 2>/dev/null || true)"
        if [[ "$current_phase" == "Pending" ]]; then
          echo "ACTIVE FailedScheduling $obj : $msg" >>"$report"
        else
          echo "INFO historical FailedScheduling $obj : $msg" >>"$report"
        fi
