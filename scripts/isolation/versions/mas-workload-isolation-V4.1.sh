...
MAS_INSTANCES="auto"; PLANS=(); ASSUME_YES=false; EXTERNAL_DB=false; FORCE_REAPPLY=false; SLS_NS=""; MONGO_NS=""; MONGO_NAME="mas-mongo-ce"; MONGO_STABLE_CHECKS=3
...
while [[ $# -gt 0 ]]; do case "$1" in
 ...
 --yes) ASSUME_YES=true; shift;; --force-reapply) FORCE_REAPPLY=true; shift;; --sls-namespace) SLS_NS="$2";shift 2;; --mongodb-namespace) MONGO_NS="$2";shift 2;; --mongodb-name) MONGO_NAME="$2";shift 2;; --mongodb-stable-checks) MONGO_STABLE_CHECKS="$2";shift 2;;
...
for c in "${COMPS[@]}"; do [[ "$c" =~ ^(mas|sls|mongodb|db2)$ ]] || { echo "Unsupported component: $c"; exit 2; }; done
...
if has_component mas || has_component sls || has_component mongodb; then
 [[ -n "$LABEL_VALUE" && -n "$ALLOWED_NODES" ]] || { echo "MAS/SLS/MongoDB selections require --label-value and --allowed-nodes"; exit 2; }
fi
...
mongodb_cr_affinity_compliant(){
  oc get mongodbcommunity "$MONGO_NAME" -n "$MONGO_NS" -o json | jq -e \
...
mongodb_sts_affinity_compliant(){
  oc get statefulset "$MONGO_NAME" -n "$MONGO_NS" -o json | jq -e \
    --arg k "$LABEL_KEY" --arg v "$LABEL_VALUE" '
...
mongodb_placement_compliant(){
  local bad
  bad="$(oc get pods -n "$MONGO_NS" -l "app=${MONGO_NAME}-svc" \
...
mongodb_phase(){
  ...
  while ((good<MONGO_STABLE_CHECKS)); do
    ph="$(oc get mongodbcommunity "$MONGO_NAME" -n "$MONGO_NS" -o jsonpath='{.status.phase}')"
    rd="$(oc get sts "$MONGO_NAME" -n "$MONGO_NS" -o jsonpath='{.status.readyReplicas}')"; rd="${rd:-0}"
    rp="$(oc get sts "$MONGO_NAME" -n "$MONGO_NS" -o jsonpath='{.spec.replicas}')"
