...
# IBM MAS / OpenShift Cluster Discovery & Assessment v1.5
...
cat <<'EOF'
Usage:
  ./mas-cluster-discovery-v1.5.sh [options]
...
Purpose:
  ...
  Detects:
    ...
    - MAS instance IDs and Core/Manage namespaces
    ...
    - dynamic Manage runtime topology (ALL/UI/MEA/CRON/REPORT/JMS)
...
mapfile -t MAS_IDS < <(
  ...
  sed -nE 's/^mas-([^-]+)-(core|manage)$/\1/p' | sort -u
)
...
# V1.5 - optional AppConfig/Graphite
...
for id in "${MAS_IDS[@]}"; do
 ...
 select(.metadata.name|startswith($id+"-maximo-"))|
...
echo '<p><b>Migration target assignments:</b> NOT YET DEFINED</p></div>'
...
echo '<div class="card"><h2>MAS Instances</h2><table><tr><th>Instance</th><th>Core Namespace</th><th>Manage Namespace</th><th>Target Workers</th></tr>'
for id in "${MAS_IDS[@]}"; do
 printf '<tr><td><b>%s</b></td><td>mas-%s-core</td><td>mas-%s-manage</td><td class="warn">NOT YET DEFINED</td></tr>\n' "$id" "$id" "$id"
done
echo '</table></div>'
...
echo '<div class="card"><h2>SLS Discovery</h2><table><tr><th>Namespace</th><th>LicenseService</th><th>Running Placement</th></tr>'
for ns in "${SLS_NS[@]}"; do
...
echo '</main><footer>IBM MAS / OpenShift Cluster Discovery v1.5 — Read-only assessment</footer></body></html>'
} >"$HTML_REPORT"
