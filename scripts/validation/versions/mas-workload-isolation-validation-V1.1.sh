...
# MAS Workload Isolation - Post Migration Validator V1.1
...
usage() {
  cat <<'EOF'
Usage:
  mas-workload-isolation-validation-V1.1.sh \
    --instance <id> \
    --core-namespace <ns> \
    [--manage-namespace <ns>] \
    [--sls-namespace <ns>] \
    [--mongodb-namespace <ns>] \
    [--facilities-namespace <ns>] \
    --label-value <value> \
...
while [[ $# -gt 0 ]]; do
  case "$1" in
    ...
    --facilities-namespace) FACILITIES_NS="$2"; shift 2;;
...
section "MAS WORKLOAD ISOLATION - POST MIGRATION VALIDATION V1.1"
...
section "FACILITIES"
if [[ -z "$FACILITIES_NS" ]]; then auto_fac="mas-${INSTANCE}-facilities"; ns_exists "$auto_fac" && FACILITIES_NS="$auto_fac"; fi
if [[ -z "$FACILITIES_NS" ]] || ! ns_exists "$FACILITIES_NS"; then
  result "Facilities application" "N/A" "optional Facilities application not installed"
else
  fac_json="$(oc --request-timeout="$OC_REQUEST_TIMEOUT" get pods -n "$FACILITIES_NS" -o json 2>/dev/null || echo '{"items":[]}')"
  f_total=0; f_bad=0; f_aff=0; f_unhealthy=0
  while IFS='|' read -r pod phase node aff; do
    [[ -n "$pod" ]] || continue
    ((f_total+=1)); [[ "$phase" == "Running" ]] || ((f_unhealthy+=1)); allowed "$node" || ((f_bad+=1)); [[ "$aff" == true ]] || ((f_aff+=1))
    echo "  $pod phase=$phase node=$node required-affinity=$aff"
  done < <(jq -r --arg k "$LABEL_KEY" --arg v "$LABEL_VALUE" '.items[]|select(.status.phase!="Succeeded")|select((.metadata.name|test("^ibm-mas-facilities-operator|^ibm-truststore-mgr-controller-manager"))|not)|[.metadata.name,.status.phase,(.spec.nodeName//""),(([.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution.nodeSelectorTerms[]?.matchExpressions[]?|select(.key==$k and .operator=="In" and (.values|index($v)!=null))]|length)>0|tostring)]|join("|")' <<<"$fac_json")
  if ((f_total==0)); then result "Facilities application" "N/A" "Facilities namespace present but no active application workloads discovered"
  elif ((f_bad>0 || f_aff>0)); then result "Facilities application" "FAIL" "active=$f_total outside-pool=$f_bad missing-required-affinity=$f_aff"
  elif ((f_unhealthy>0)); then result "Facilities application" "WARN" "placement/affinity compliant but $f_unhealthy active workload(s) not Running"
  else result "Facilities application" "PASS" "$f_total active workload(s) Running on target pool with required affinity"; fi
  ...
  for ws in "${_fws[@]}"; do
    ready="$(oc get facilitiesworkspace "$ws" -n "$FACILITIES_NS" -o json | jq -r '[.status.conditions[]?|select(.type=="Ready")][0].status // "Unknown"')"
    valid="$(oc get facilitiesworkspace "$ws" -n "$FACILITIES_NS" -o json | jq -r '[.status.conditions[]?|select(.type=="PodTemplatesValid")][0].status // "Unknown"')"
    echo "  workspace=$ws Ready=$ready PodTemplatesValid=$valid"
    [[ "$ready" == True && "$valid" == True ]] || result "Facilities workspace $ws" "FAIL" "Ready=$ready PodTemplatesValid=$valid"
  done
  echo "Supporting Facilities controllers (informational):"
  jq -r '.items[]|select(.status.phase=="Running")|select(.metadata.name|test("^ibm-mas-facilities-operator|^ibm-truststore-mgr-controller-manager"))|[.metadata.name,.spec.nodeName]|@tsv' <<<"$fac_json" || true
fi
