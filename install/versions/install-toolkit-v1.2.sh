...
step "IBM MAS WORKLOAD ISOLATION TOOLKIT INSTALLER V${INSTALLER_VERSION}"
...
step "PREPARE LOCAL TOOLKIT FOR COPY"

# macOS attaches metadata such as com.apple.quarantine to downloaded files.
# Remove those attributes when xattr is available so Linux tar in the MAS CLI
# pod does not emit LIBARCHIVE.xattr.* warnings. Failure to remove a noncritical
# attribute must not block installation.
if command -v xattr >/dev/null 2>&1; then
  echo "[INFO] Removing macOS extended attributes from local toolkit..."
  xattr -rc "$PROJECT_ROOT" 2>/dev/null || true
  pass "macOS extended-attribute cleanup completed"
else
  echo "[INFO] xattr not available; skipping macOS metadata cleanup"
fi

# COPYFILE_DISABLE prevents AppleDouble files on macOS. The tar exclusions keep
# development metadata and generated run/report artifacts out of the MAS CLI pod.
TAR_EXCLUDES=(
  "--exclude=.git"
  "--exclude=.git/*"
  "--exclude=.DS_Store"
  ...
  "--exclude=*/._*"
  "--exclude=*.tar.gz"
  "--exclude=cluster-discovery-report-*"
  "--exclude=mas-workload-validation-*"
  "--exclude=mas-workload-isolation-*-20????????-??????"
)

echo "Local project : $PROJECT_ROOT"
echo "Remote project: $REMOTE_ROOT"
echo "Excluded from copy:"
printf '  %s\n' \
  ".git/" \
  ".DS_Store / AppleDouble metadata" \
  "*.tar.gz" \
  "cluster-discovery-report-*" \
  "mas-workload-validation-*" \
  "timestamped mas-workload-isolation run directories"
...
# COPYFILE_DISABLE=1 suppresses macOS AppleDouble metadata generation.
COPYFILE_DISABLE=1 tar -C "$PROJECT_ROOT" "${TAR_EXCLUDES[@]}" -cf - . | \
...
# Defensive cleanup in case metadata existed in an older persistent workspace.
oc exec -n "$NAMESPACE" "$POD" -c "$CONTAINER" -- bash -c "
  find '$REMOTE_ROOT' -type f \( -name '.DS_Store' -o -name '._*' \) -delete 2>/dev/null || true
"
pass "Development metadata cleanup completed"
...
step "VERIFY CLEAN TOOLKIT COPY"
oc exec -n "$NAMESPACE" "$POD" -c "$CONTAINER" -- bash -c "
  set -e
  [ ! -e '$REMOTE_ROOT/.git' ] || { echo '[FAIL] .git directory was copied'; exit 1; }
  if find '$REMOTE_ROOT' -type f \( -name '.DS_Store' -o -name '._*' \) -print -quit | grep -q .; then
    echo '[FAIL] macOS metadata files were copied'
    exit 1
  fi
  echo '[PASS] Git metadata excluded'
  echo '[PASS] macOS metadata excluded'
"
